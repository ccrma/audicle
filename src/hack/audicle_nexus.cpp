...
struct commandline { int argc; char ** argv; };
...
t_CKBOOL chuck_chuck( int argc, char ** argv )
{
    ...
    commandline * a = new commandline; a->argc = argc; a->argv = argv;
                       ...
                       FILE * fd )
{
    ...
    {
        fprintf( stderr, "[chuck]: unrecognized command from audicle: '%i'\n", cmd->type );
        SAFE_DELETE(cmd);
        return 0;
    }
...
// desc: ...
//-----------------------------------------------------------------------------
t_CKBOOL parse( c_str fname, FILE * fd )
{
    t_CKBOOL ret = FALSE;
...
int main2( int argc, char ** argv )
{
    ...
    if( !vm->initialize( enable_audio, vm_halt, srate, 2048,
                         8, dac, adc, g_priority ) )
    {
