...
t_CKUINT AudicleFaceTnT::render( void * d )
{
    ...
    AudicleStats::instance()->get_shreds( m_shreds, m_done );
    Shred_Stat * stat;    
...
AudicleFaceTnT::render_cycle_heatmap() { 
    ...
    render_cycle_shred_group( top, 0, top.size(), m_aspt * 2.0 , 1.9);  
    glPopMatrix();
    return top.size();
}
...
AudicleFaceTnT::recalculate_scaling_factors () { 
    ...
    double inactive_h  =  max ( 0, ( screen_h - actbase * tmp_active_scale ) ); 
...
{
    AudicleStats::activations_yes = TRUE;
...
{
    AudicleStats::activations_yes = FALSE;
